...
const int MOTION_THRESHOLD = 4000;
...
void image_processing_task(uint8_t* jpg_buf, size_t jpg_buf_len) {
    ...
    if (decode_res == ESP_OK) {
        ...
        if (is_first_frame) {
            ...
            for (int i = 0; i < SCALED_IMAGE_WIDTH * SCALED_IMAGE_HEIGHT; i++) {
                grayscale_sum += grayscale_img_buf[i];
                reference_sum += reference_frame_buf[i];
            }
