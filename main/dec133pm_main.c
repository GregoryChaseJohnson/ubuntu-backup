...
#define SCALED_IMAGE_WIDTH 80  // Width for 80x60 image
#define SCALED_IMAGE_HEIGHT 60 // Height for 80x60 image

#define RGB565_BYTES_PER_PIXEL 2 // 2 bytes per pixel for RGB565 format

#define DECODING_FPS 1 // or any other appropriate frame rate value

static uint8_t out_img_buf[SCALED_IMAGE_WIDTH * SCALED_IMAGE_HEIGHT * RGB565_BYTES_PER_PIXEL];

#define CONFIG_XCLK_FREQ 20000000 
#define DESIRED_FPS 7
#define DESIRED_FRAME_TIME_MS (200 / DESIRED_FPS)
#define FILE_SIZE_BUFFER_LENGTH 25
#define FILE_SIZE_CHANGE_THRESHOLD 300 // Adjust this value HIGHER to reduce false postives 
...
esp_err_t jpg_stream_httpd_handler(httpd_req_t *req) {
    ...
    const int64_t decodingInterval = 1000000 / DECODING_FPS;
    int64_t lastDecodingTime = 0;

    while (true) {
        ESP_ERROR_CHECK(esp_task_wdt_reset());
        int64_t frame_start_time = esp_timer_get_time();
        //erase
        int64_t fb_get_start_time = esp_timer_get_time();
        fb = esp_camera_fb_get();
        
        //erase both
        int64_t fb_get_end_time = esp_timer_get_time();
        int64_t fb_get_duration = fb_get_end_time - fb_get_start_time;
        
        //erase
        ESP_LOGI(TAG, "esp_camera_fb_get() took %lld microseconds", fb_get_duration);
        ...
        // Use the frame buffer directly if it is in JPEG format
        _jpg_buf_len = fb->len;
        _jpg_buf = fb->buf;

        // Execute the JPEG decoding logic independently of motion detection
        if (frame_start_time - lastDecodingTime >= decodingInterval) {
            lastDecodingTime = frame_start_time;

            // JPEG Decoding Logic
            esp_jpeg_image_cfg_t jpeg_cfg = {
                .indata = _jpg_buf,
                .indata_size = _jpg_buf_len,
                .outbuf = out_img_buf,
            ...
            if (decode_res == ESP_OK) {
                ...
                for (int i = 0; i < outimg.width * outimg.height; ++i) {
                    pixel_sum += ((uint16_t*)out_img_buf)[i];
                }
                ESP_LOGI(TAG, "Sum of pixel values: %" PRIu32, pixel_sum);
            } else {
                ESP_LOGE(TAG, "JPEG Decoding Failed");
            }
        }

        // Motion detection logic using the same frame buffer
