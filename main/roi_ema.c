    ...
    if (decode_res == ESP_OK) {
        ...
        if (is_first_frame) {
            ...
            is_first_frame = false;
             ...
             for (int x = 0; x < ROI_WIDTH; x += 2) { // Every other column in ROI
                for (int y = ROI_START_Y; y < ROI_END_Y; y++) { // Rows within ROI
                    int index = y * IMAGE_WIDTH + x;
                    reference_frame_buf[index] = (uint8_t)((1.0 - EMA_WEIGHT) * grayscale_img_buf[index] + EMA_WEIGHT * reference_frame_buf[index]);
                }
            }
            ...
            // Example thresholds
            int change_threshold = 90                              ;  // Define based on your application
            int min_contiguous = 2;      // Number of contiguous columns indicating motion
