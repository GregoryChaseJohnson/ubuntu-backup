...
uint8_t previous_img_buf[4800];
static const char *TAG = "esp32";

#define FRAME_CAPTURE_INTERVAL_MS 150
...
const int MAX_CONSECUTIVE_MOTION_FRAMES = 5;
...
void image_processing_task(uint8_t *jpg_buf, size_t jpg_buf_len)
{
    ...
    if (decode_res == ESP_OK)
    {
        ...
        if (is_first_frame)
        {
            memcpy(reference_frame_buf, current_img_buf, sizeof(reference_frame_buf));
        ...
        else
        {
            ...
            int change_threshold = 1200; // Define based on your application
            int min_contiguous = 5;
            ...
            { // Every other pixel in width
                int column_change_sum = 0;
                for (int y = ROI_START_Y; y < ROI_END_Y; y++)
                {
                    int index = y * IMAGE_WIDTH + x;
                    column_change_sum += abs(current_img_buf[index] - reference_frame_buf[index]);
                }
            ...
            int change_threshold_strips = 1200; // Define based on your application for strip changes
            int min_contiguous_strips = 3;

            // strip chnage logic
            int num_strips = (ROI_WIDTH + 1) / 2;
            ...
            for (int x = 0; x < ROI_WIDTH; x += 2)
            {
                ...
                for (int strip_x = x; strip_x < x + 2 && strip_x < ROI_WIDTH; strip_x++)
                {
                    for (int y = 0; y < ROI_HEIGHT; y += 2)
                    {
                        int index = y * IMAGE_WIDTH + strip_x;
                        strip_change_sum += abs(current_img_buf[index] - reference_frame_buf[index]);
                    }
                }

                strip_changes[x / 2].sum = strip_change_sum;
                strip_changes[x / 2].changed = false;
            }
            ...
            else
            {
                ...
                for (int x = 0; x < ROI_WIDTH; x += 2)
                {
                    ...
                    {
                        int index = y * IMAGE_WIDTH + x;
                        reference_frame_buf[index] = (current_img_buf[index] + previous_img_buf[index] + reference_frame_buf[index]) / 3;
                    }
                }
            }

            memcpy(previous_img_buf, current_img_buf, sizeof(previous_img_buf));
        }
    }
    else
    {
