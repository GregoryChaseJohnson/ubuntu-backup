...
void image_processing_task(uint8_t* jpg_buf, size_t jpg_buf_len) {
    ...
    if (decode_res == ESP_OK) {
        ...
        if (is_first_frame) {
            ...
            uint8_t* diff_frame = malloc(ROI_WIDTH * ROI_HEIGHT * sizeof(uint8_t));
            if (diff_frame == NULL) {
                ESP_LOGE(TAG, "Failed to allocate memory for diff_frame");
            ...
            for (int y = ROI_START_Y; y < ROI_END_Y; y++) {
                for (int x = 0; x < SCALED_IMAGE_WIDTH; x++) {
                    ...
                    diff_frame[index] = abs(grayscale_img_buf[index] - reference_frame_buf[index]) > MOTION_THRESHOLD ? 1 : 0;
                    diff_sum += diff_frame[index];
                }
            }
